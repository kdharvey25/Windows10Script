# Ricker here, Imma just say that I have modified this script to my likeing, but overall it belongs to Evan Greene

# FYI Evan, Love the script dude, very helpful for policys, only hate is that there is a lot for one script
# Along with, ya know, modding this to my needs :)
...
# Modded by Ricker Wilkes in https://github.com/HungerGames682/CyberPatriot

##########################################################################################################
$LogonLegalNoticeMessageTitle = "Hello, Welcome to the Server"
#$LogonLegalNoticeMessage = "Ricker is your god now, you must obey him and his commands. If you do not, you will be punished. Have a nice day!"
# The message above github copilot made on its own
...
$NewLocalAdmin = [System.Environment]::UserName # Active admin account username (Local admin account that will be used to manage the server. Account will be active after script is run. This is not a prefix. It's the full account username)

#########################################################
# NOTE: Ricker here, Ummmm, I knows its document well, but its a little difficult to understand, so imma just run and see where it takes me
...
$ExecutionList = @(
    ...
    # "CreateNewLocalAdminAccount",                                       #Mandatory otherwise the system access is lost, HA we will see about that Evan
...
# Ricker Here again, Of course I know what im doing, I am a professional. :)
##########################################################################################################

# NOTE: Don't mess with this part in particular, don't know how or why, but the script just won't continue if you get rid of it
...
# Im kind enough to keep the credists in the script
Write-Info "CIS Microsoft Windows Server 2022 Benchmark"
Write-Info "Script by Evan Greene, Modified by Ricker Wilkes"
...
$location = "./Logs/PolicyResults"

# Will create the folder if it does not exist
if (-not (Test-Path -Path $location)){
    New-Item -ItemType Folder -Path $location | Out-Null
}
...
function Test-WiFiConnection {
    try {
        $result = Test-Connection -ComputerName 8.8.8.8 -Count 1 -ErrorAction Stop
        return $true
    } catch {
        return $false
    }
...
$start = 0
$end = $ExecutionList.Count - 1
$commandThatDisablesWiFi = $null

while ($start -le $end) {
    $mid = [math]::Floor(($start + $end) / 2)
    $command = $ExecutionList[$mid]

    Invoke-Expression $command | Out-File $location\CommandsReport.txt -Append

    if (-not (Test-WiFiConnection)) {
        Write-Host "WiFi disabled after executing: $command"
        $commandThatDisablesWiFi = $command
        $end = $mid - 1 # Narrow down the search to the left half
    } else {
        $start = $mid + 1 # Narrow down the search to the right half
    }

    # Assuming you have a way to re-enable WiFi here for the next iteration
}

if ($null -ne $commandThatDisablesWiFi) {
    Write-Host "The command that disables WiFi is: $commandThatDisablesWiFi"
} else {
    Write-Host "No command found that disables WiFi."
}
